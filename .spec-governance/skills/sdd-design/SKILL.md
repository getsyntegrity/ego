...
This is where the detail `sdd-propose` is required to defer lands: full type/interface/method signatures, the exact algorithm/library/tooling choice behind any mechanism the proposal only named, and per-decision rationale beyond a single line. If `proposal.md` names something it deferred (a "Deferred to design" list in its summary, or a bare concept like "a `TenantResolver` SPI"), this is the phase that fixes its concrete shape.
