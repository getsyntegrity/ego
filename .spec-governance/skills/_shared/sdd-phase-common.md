...
**`state.yaml` writes** (`openspec`/`hybrid`): before marking a phase complete that writes to `state.yaml`, run the canonical **State Hygiene Gate** defined in `skills/_shared/openspec-convention.md` (`state.yaml Discipline`). Its failure blocks phase completion — do not mark the phase done with a known violation.
...
## G. Spec Governance Gate

`spec-governance`, `spec-splitting`, `spec-authoring`, and `spec-evidence` are portable, tool-independent skills (`spec-*`, siblings of the `sdd-*` skills in this same package) that answer a question the phase templates below don't: is this change actually ONE atomic unit, and is its content actually proven, not just written? Load them by trigger, same as any other skill in Section A — they are not sdd-specific and don't know about `openspec`/`engram` artifact stores.

- **Atomicity, at `sdd-explore` and `sdd-propose`**: before recommending "Ready for Proposal: Yes" (explore) or finalizing scope (propose), run `spec-governance`'s decision process against the change's declared outcome. `ATOMIC` → proceed. `REVIEW_REQUIRED` → surface the specific semantic question (governance §8) to the orchestrator/user instead of guessing; do not silently pick a side. `SPLIT_REQUIRED` → STOP; the change needs `spec-splitting` before a single proposal/design/tasks set makes sense. This is exactly the failure mode that produces a proposal doing three phases' work at once — catch it at explore, not after.
- **Structure, across `proposal.md`/`spec.md`/`design.md`/`tasks.md`**: `spec-authoring` defines one spec's required sections (Outcome, Scope, Requirements, AC, Constraints, Contracts, Design, Tasks) as a single conceptual document. Ego splits that same content across phase files instead of one file — `proposal.md` owns Outcome/Scope, `spec.md` owns Requirements/AC/Contracts, `design.md` owns Constraints/Design, `tasks.md` owns Tasks. When authoring any of these, apply `spec-authoring`'s per-section rules (§6 concrete-outcome requirement, §9 one-verifiable-behavior-per-requirement, §11 requirement/design/task separation, §20 bloat prevention) to the section it maps to, not the sdd-propose/sdd-design Content Boundary alone — the two are complementary, not redundant.
- **Human gates**: `spec-governance` §10 is the single canonical list (architecture, security, public API, data migrations, irreversible operational changes). When any phase artifact records a required human approval, use these categories — do not invent new ones locally.
- **Evidence, at `sdd-verify`**: reason about spec-scenario compliance using `spec-evidence`'s vocabulary — an AC/scenario is `PROVEN` (sufficient, current, direct evidence), `NOT_PROVEN` (implementation exists, evidence doesn't), `BLOCKED`, or `NOT_APPLICABLE` (only via an approved spec change, never to hide a failure). For security/enforcement scenarios, positive evidence alone (`valid input accepted`) is insufficient without the matching negative-path evidence (`invalid input rejected`) — `spec-evidence` §9. This does not change `sdd-verify`'s report format or byte-exact envelope contract; it's the reasoning standard behind the compliance matrix it already produces.
- **Re-evaluation**: per `spec-governance` §11, re-run the atomicity check whenever a change's scope grows after the initial verdict (new requirement, new AC, new subsystem, resumed after being stale) — not just once at explore.
- **`state.yaml` is metadata, not a phase file**: it is the state machine + artifact index + decision/gate IDs — never a fifth place narrative/evidence/rationale gets restated. See `state.yaml Discipline` in `skills/_shared/openspec-convention.md` for exactly what belongs there versus in `exploration.md`/`proposal.md`/`design.md`. Any phase that persists to `state.yaml` (Section C) enforces this at write time.
