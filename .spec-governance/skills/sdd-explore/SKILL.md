...
### Step 4a: Atomicity Check

Follow **Section G** (Spec Governance Gate) from `skills/_shared/sdd-phase-common.md`. Run `spec-governance`'s decision process against the recommended approach's outcome before answering "Ready for Proposal" in Step 6. A `SPLIT_REQUIRED` or `REVIEW_REQUIRED` verdict overrides a "Yes" answer — see Step 6.
...
### Atomicity
{ATOMIC | REVIEW_REQUIRED: {semantic question} | SPLIT_REQUIRED: {why, and the candidate split}}

### Ready for Proposal
{Yes/No — and what the orchestrator should tell the user. No when Atomicity is SPLIT_REQUIRED or REVIEW_REQUIRED.}
```
