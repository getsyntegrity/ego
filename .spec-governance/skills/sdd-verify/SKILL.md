...
- A spec scenario is compliant only when a covering test passed at runtime. Follow **Section G** (Spec Governance Gate) from `../_shared/sdd-phase-common.md` for the PROVEN/NOT_PROVEN/BLOCKED/NOT_APPLICABLE vocabulary and the positive+negative-path evidence rule for security/enforcement scenarios; this does not change the report format below.
