...
- [ ] 3.9 Test: handler's received arguments contain no `ExpectedRevision`, `event_sourced_actor_test.go`. Req: ExpectedRevision Extracted for the Persist Request Only. NOT marked done: no dedicated test found asserting this on the handler's received arguments specifically (verified by code audit during the PR #86 D9 review, 2026-09-17) — the invariant holds architecturally (`HandleCommand`/`HandleEnvelope` signatures carry no precondition parameter) but lacks its own explicit test per this task's wording.
